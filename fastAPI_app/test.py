...
def load_model_and_vectorizer(
    model_name: str, model_version: str, vectorizer_path: str
) -> Tuple[Any, Any]:
    """
...
def test_predict(
    model: Any, vectorizer: Any, test_comments: List[str] = None
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        preprocessed_comments = [
            preprocess_comment(comment) for comment in test_comments
        ]
...
def print_results(results: List[Dict[str, Any]]) -> None:
    ...
    for i, result in enumerate(results, 1):
        ...
        print(
            f"   Sentiment: {result['sentiment_label']} ({result['sentiment']}) {emoji}"
        )
