...
def clean_data(df: pd.DataFrame, dataset_name: str = "dataset") -> pd.DataFrame:
    ...
    logger.info(f"  Total removed: {rows_removed} ({100*rows_removed/initial_rows:.2f}%)")
...
def normalize_text(df: pd.DataFrame, dataset_name: str = "dataset") -> pd.DataFrame:
    ...
    try:
        ...
        if empty_after > 0:
            logger.warning(f"Found {empty_after} comments that became empty after preprocessing")
        ...
        logger.info(f"  Reduction: {100*(avg_length_before-avg_length_after)/avg_length_before:.1f}%")
