...
def apply_tfidf(
    ...
    try:
        ...
        logger.info(f"  Sparsity: {100 * (1 - X_train_tfidf.nnz / (X_train_tfidf.shape[0] * X_train_tfidf.shape[1])):.2f}%")
...
# ==================== MLFLOW INTEGRATION ====================
def save_experiment_info(run_id: str, model_path: str, metrics: Dict, params: Dict, root_dir: Path):
...
def train_with_mlflow(root_dir: Path, params: Dict):
    ...
    with mlflow.start_run(run_name=f"training_{datetime.now().strftime('%Y%m%d_%H%M%S')}") as run:
        ...
        try:
            ...
            mlflow.log_param("target_distribution", str(data_stats['target_distribution']))
            ...
            sample_indices = np.random.choice(len(train_data), size=min(5, len(train_data)), replace=False)
            ...
            signature = infer_signature(input_example_tfidf, model.predict(input_example_tfidf))
            ...
            mlflow.sklearn.log_model(
                ...
                signature=signature,
                input_example=input_example_tfidf[:2].toarray(),  # Log first 2 samples as example
