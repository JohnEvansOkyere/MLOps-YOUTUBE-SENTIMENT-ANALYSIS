...
def evaluate_model(
    model, X_test: np.ndarray, y_test: np.ndarray, dataset_name: str = "test"
    ...
    try:
        ...
        for label, label_metrics in report_dict.items():
            if isinstance(label_metrics, dict) and label not in [
                "accuracy",
                "macro avg",
                "weighted avg",
            ]:
...
def save_confusion_matrix(
    cm: np.ndarray, dataset_name: str, class_names: list = None
) -> str:
...
def save_metrics_report(metrics: Dict, report_dict: Dict, dataset_name: str) -> str:
...
def save_latest_artifacts(cm_path: str, metrics_path: str):
    ...
    try:
        ...
        cm_latest = (
            reports_dir / "confusion_matrices" / "confusion_matrix_test_latest.png"
        )
...
def run_drift_detection(
    ...
    try:
        ...
        drift_detected, drift_summary, drift_report = detector.detect_drift(
            reference_data=reference_data, current_data=current_data, save_report=True
        )
...
def run_quality_check(
    data: pd.DataFrame, config_path: str = "params.yaml"  # Add this parameter
) -> Tuple[bool, Dict]:
...
def save_evaluation_info(
    run_id: str, metrics: Dict, drift_summary: Dict, quality_summary: Dict
) -> Dict:
    ...
    try:
        ...
        if training_info_path.exists():
            ...
            training_info["evaluation"] = {
                ...
                "monitoring": {"drift": drift_summary, "quality": quality_summary},
                "timestamp": datetime.now().isoformat(),
            }
            ...
            raise FileNotFoundError(
                "Training experiment_info.json not found! Run training first."
            )
...
def main():
    ...
    try:
        ...
        with mlflow.start_run(
            ...
            quality_passed, quality_summary = run_quality_check(
                test_data, config_path=params_config_path
            )
            ...
            if reference_data_path.exists():
                ...
                logger.warning(
                    "Skipping drift detection. Run: python scripts/create_reference_data.py"
                )
            ...
            report_gen.log_to_mlflow(
                drift_summary=drift_summary, quality_summary=quality_summary
            )
            ...
            # Log vectorizer
            mlflow.log_artifact(
                str(root_dir / "tfidf_vectorizer.pkl"), artifact_path="vectorizer"
            )
