...
AUTO_PROMOTE_TO_PRODUCTION = os.getenv('AUTO_PROMOTE_TO_PRODUCTION', 'false').lower() == 'true'
...
PERFORMANCE_IMPROVEMENT_THRESHOLD = float(os.getenv('PERFORMANCE_IMPROVEMENT_THRESHOLD', '0.02'))
...
def load_model_info(file_path: str = 'experiment_info.json') -> Dict:
    ...
    try:
        ...
        if missing_fields:
            raise ValueError(f"Missing required fields in model_info: {', '.join(missing_fields)}")
...
# ==================== MODEL COMPARISON ====================
def get_current_production_model(client: MlflowClient, model_name: str) -> Optional[Dict]:
    ...
    try:
        ...
        if "RESOURCE_DOES_NOT_EXIST" in str(e):
            logger.info(f"Model '{model_name}' not found in registry. This will be the first version.")
            return None
        raise

def compare_model_performance(new_metrics: Dict, prod_metrics: Optional[Dict]) -> Tuple[bool, str]:
    """
    Compare new model metrics with production model.
    Returns (should_promote, reason)
    """
    if prod_metrics is None:
        return True, "No production model exists - auto-promoting first model to staging"
    ...
    if new_accuracy < MIN_ACCURACY_THRESHOLD:
        return False, f"Accuracy {new_accuracy:.4f} below threshold {MIN_ACCURACY_THRESHOLD}"
    ...
    if accuracy_improvement >= PERFORMANCE_IMPROVEMENT_THRESHOLD or f1_improvement >= PERFORMANCE_IMPROVEMENT_THRESHOLD:
        return True, f"Performance improved (Acc: +{accuracy_improvement:.4f}, F1: +{f1_improvement:.4f})"
    
    if accuracy_improvement >= 0 and f1_improvement >= 0:
        return True, "Performance maintained or slightly improved"
    
    return False, f"Performance degraded (Acc: {accuracy_improvement:.4f}, F1: {f1_improvement:.4f})"
...
def register_model_to_mlflow(
    ...
    try:
        ...
        if not validate_model_artifact(client, model_info['run_id'], model_info['model_path']):
        ...
        should_promote, reason = compare_model_performance(
            new_metrics,
            prod_model['metrics'] if prod_model else None
        )
        ...
        # Add metadata
        add_model_metadata(client, model_name, model_version.version, model_info, new_metrics)
        ...
        if should_promote:
            ...
            if AUTO_PROMOTE_TO_PRODUCTION:
                ...
                transition_model_stage(client, model_name, model_version.version, "Production")
