...
class DataDriftDetector:
    ...
    def prepare_data(
        self, df: pd.DataFrame, text_column: str = "clean_comment"
    ) -> pd.DataFrame:
        ...
        df["uppercase_ratio"] = df[text_column].str.count("[A-Z]") / (
            df["text_length"] + 1
        )
        df["digit_ratio"] = df[text_column].str.count("[0-9]") / (df["text_length"] + 1)
        df["special_char_ratio"] = df[text_column].str.count("[^a-zA-Z0-9\s]") / (
            df["text_length"] + 1
        )
    ...
    def detect_drift(
        ...
        report = Report(
            metrics=[
                DataDriftPreset(
                    stattest="wasserstein", stattest_threshold=self.drift_threshold
                ),
                DataQualityPreset(),
            ]
        )
        ...
        n_drifted_features = drift_results["metrics"][0]["result"][
        ...
        drift_summary = {
            ...
            "drift_score": drift_results["metrics"][0]["result"][
