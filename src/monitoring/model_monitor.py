...
class ModelPerformanceMonitor:
    ...
    def __init__(self, config_path: str = "params.yaml"):
        ...
        self.accuracy_threshold = self.config["monitoring"]["performance"][
            "accuracy_threshold"
        ]
        self.f1_threshold = self.config["monitoring"]["performance"]["f1_threshold"]
        self.degradation_threshold = self.config["monitoring"]["performance"][
    ...
    def monitor_performance(
        ...
        report.run(
            ...
            column_mapping={"target": target_col, "prediction": prediction_col},
        )
        ...
        degradation_detected = (
            current_metrics["accuracy"] < self.accuracy_threshold
            or current_metrics["f1_weighted"] < self.f1_threshold
            or accuracy_drop > self.degradation_threshold
            or f1_drop > self.degradation_threshold
        )
